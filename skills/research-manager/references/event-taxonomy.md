# Event Taxonomy & Routing Rules — LiDAR Perception Edition
...
### Domain-specific direct routing

| Type | Signals | Target | Required payload |
|------|---------|--------|------------------|
| `benchmark_run` | Model evaluated on KITTI/Waymo/nuScenes/Argoverse2 | `evidence/benchmarks/` + `trace/checkpoints.yaml` | dataset, split, metrics dict, checkpoint ref |
| `ablation_result` | Controlled variable experiment completed | `evidence/ablation/` | dimension, variable, baseline vs. variant metrics |
| `dataset_decision` | Split choice, augmentation strategy, preprocessing decision | `trace/exploration_tree.yaml` as `decision` | rationale, affected experiments |
| `sensor_config` | Sensor choice or parameter fixed | `trace/exploration_tree.yaml` as `decision` | sensor model, parameters, rationale |
...
|-----------------|---------|-----------------|------------------|
| `claim` | "I believe...", "The model achieves...", falsifiable assertion about capability/property | `logic/claims.md` | `claim` |
...
### Domain-specific staged routing

| Candidate Event | Signals | Crystallizes To | `potential_type` |
|-----------------|---------|-----------------|------------------|
| `sensor_constraint` | "This only works at X meters with Y lines..." | `logic/solution/constraints.md` as SC{XX} | `constraint` |
| `weather_degradation` | "In rain > X mm/hr, performance drops by Y..." | `logic/solution/constraints.md` as EC{XX} | `constraint` |
| `latency_requirement` | "This needs to run at X Hz on Y hardware..." | `logic/solution/constraints.md` as DC{XX} | `constraint` |
| `augmentation_heuristic` | "GT sampling with N objects works best because..." | `logic/solution/heuristics.md` | `heuristic` |
| `voxelization_heuristic` | "The right pillar size for this sensor is X because..." | `logic/solution/heuristics.md` | `heuristic` |

Evidence artifacts referenced by a direct-routed `experiment` or `benchmark_run` get
written to `evidence/` immediately — they are raw data, not interpretation.

**Benchmark evidence files** naming convention:
`evidence/benchmarks/{dataset}_{split}_{metric_focus}.md`

**Ablation evidence files** naming convention:
`evidence/ablation/{dimension}_{variable}.md`
...
What KIND of event is this?
  ...
  Raw evidence (data produced)?
    Standard benchmark (KITTI/Waymo/nuScenes/Argoverse2) on a known split?
      → benchmark_run  [DIRECT to evidence/benchmarks/ + trace/checkpoints.yaml]
    Controlled variable experiment (ablation)?
      → ablation_result  [DIRECT to evidence/ablation/]
    Other quantitative result (table, figure, metric time-series)?
      → route to evidence/tables/ or evidence/figures/ as appropriate

  Journey fact (something that happened)?
    Was a choice made between alternatives?
      → decision  [DIRECT to trace/]
        - Dataset/split/augmentation choices → tag with "dataset" in title
        - Sensor/config choices → tag with "sensor" in title
  ...
  Interpretation (something asserted to be true / general)?
    Falsifiable assertion about the model/method?
    ...
    Sensor/environment/deployment limitation?
...
- Routine `pip install`, `apt-get`, CUDA/cuDNN setup
...
This is especially important in safety-critical domains like autonomous driving perception,
where unverified AI-suggested claims about detection performance or sensor robustness
...
| Sensor constraint | SC | SC01, SC02 | Global; assigned at crystallization |
| Environment constraint | EC | EC01, EC02 | Global; assigned at crystallization |
| Deployment constraint | DC | DC01, DC02 | Global; assigned at crystallization |
| Checkpoint | CKPT | CKPT01, CKPT02 | Global; assigned at save time |
...
- **Claim → Proof**: at crystallization, what evidence supports/refutes it?
  - In LiDAR perception: which benchmark result, ablation study, or analysis?
- **Experiment → Claim**: which staged or crystallized claim does this experiment test?
  This binding enables the **empirical-resolution** closure signal.
...
- **Observation → Bound nodes**: at staging time, list `bound_to: [N{XX}, ...]` for any
  exploration nodes the observation depends on.
- **Constraint → Claims**: which claims are bounded by this constraint?
- **Checkpoint → Claims**: which claims' supporting evidence comes from this checkpoint?
- **Benchmark → Checkpoint**: which model checkpoint produced this benchmark result?
