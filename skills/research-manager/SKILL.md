...
description: |
  ...
  writes them into the ara/ artifact through a four-stage pipeline: Context Harvester → Event
  Router → Maturity Tracker → Logic Layer Reconciliation.

  Adapted for LiDAR perception / autonomous driving research:
  - Adds benchmark/ablation evidence routing for KITTI, Waymo, nuScenes-style evaluation
  - Refines constraints into sensor, environment, and deployment categories
  - Tracks model checkpoints, dataset splits, and augmentation configs as first-class evidence
user-invocable: true
argument-hint: "[optional: hint about what happened this turn]"
allowed-tools: Read, Write, Edit, Glob, Grep
metadata:
  author: ara-commons (adapted for LiDAR perception)
  version: "2.2.0"
  tags: [research, process-recording, provenance, progressive-crystallization, lidar, autonomous-driving, 3d-object-detection]
---

# Live Research Project Manager (Live PM) — LiDAR Perception Edition
...
This edition is adapted for LiDAR-based 3D perception research — point cloud processing,
autonomous driving benchmarks, sensor-specific constraints, and model training workflows.
...
- **AI actions performed**: experiment runs, code edits, file creations, commands,
  literature searches, benchmark numbers, model training runs, dataset processing.
- **Researcher directions** expressed or confirmed: hypotheses, design choices, abandoned
  approaches, questions, affirmations, revisions, sensor configuration decisions.
...
#### Domain-specific routing additions

- **Benchmark results** (KITTI mAP, Waymo mAPH, nuScenes NDS, etc.) → write to
  `evidence/benchmarks/` immediately as raw data, then reference from experiment nodes.
- **Ablation results** (sensor config variations, weather condition breakdowns, component
  removal studies) → write to `evidence/ablation/` immediately.
- **Model checkpoint metadata** (epoch, loss, metrics at save point) → append to
  `trace/checkpoints.yaml`.
- **Dataset split / augmentation decisions** → route as `decision` nodes with dataset
  provenance.
...
3. **Empirical resolution** — an experiment in the observation's `bound_to` produced a
   ...
   For LiDAR perception: this includes benchmark evaluation completion, ablation study
   results, sensor comparison data, and weather-condition breakdowns.
...
```
1. Read existing ara/ files for current state (next IDs, claims, tree, staging, checkpoints).
...
3. Stage 2 — Event Router: for each candidate, per references/event-taxonomy.md:
     ...
     benchmark/ablation data → write to evidence/benchmarks/ or evidence/ablation/ immediately
...
9. Record skill usage trace to `~/.claude/projects/-home-glc/memory/skill-evolution/traces/turns/YYYY-MM-DD.yaml`
   (append to today's file). For each skill invoked this turn, record:

   ```yaml
   - turn_id: "YYYY-MM-DDTHH:MM"
     skills_invoked:
       - name: <skill-name>
         trigger: "<user's triggering phrase>"
         completion: completed | aborted | corrected | failed
         user_signal: satisfied | corrected | repeated | unclear
     no_skill_gap: null | "<user request that matched no skill>"
   ```

   Rules for signal detection:
   - `completion: completed` — skill executed to its natural end
   - `completion: aborted` — user interrupted execution mid-way
   - `completion: corrected` — user corrected the skill's behavior, then it completed
   - `completion: failed` — skill execution errored or was abandoned
   - `user_signal: satisfied` — user proceeded to next task without correction
   - `user_signal: corrected` — user explicitly pointed out an error
   - `user_signal: repeated` — user re-issued a similar request after first attempt
   - `user_signal: unclear` — cannot determine satisfaction from this turn alone
   - `no_skill_gap` — if the user asked for something that no skill covered but could have, record the verbatim request

   Do NOT record this step if the turn was skipped (no research events). The
   trace file is append-only — never edit prior entries.

   After recording, check whether to auto-trigger skill-evolution analysis:

   a. Read `~/.claude/projects/-home-glc/memory/skill-evolution/state.yaml`.
      If the file does not exist, create it with:
      ```yaml
      turn_count_since_last_analysis: 1
      last_analysis_date: null
      analysis_interval: 10
      ```
      and skip the analysis (not enough data yet).
   b. If the file exists, increment `turn_count_since_last_analysis` by 1 and write back.
   c. If `turn_count_since_last_analysis >= analysis_interval`:
      - Run the skill-evolution analysis pipeline:
        i.   Read all Layer 1 traces under `traces/turns/` accumulated since last analysis
        ii.  Read Layer 2 traces for skills with instrumentation
        iii. Run the Four Diagnostic Dimensions (per skill-evolution SKILL.md section 3):
             Trigger Gap / Process Drift / Information Gap / Redundancy
        iv.  For each diagnosis meeting the confidence threshold, generate a proposal
             file in `proposals/` (per skill-evolution SKILL.md section 5 template).
        v.   For each proposal, classify risk level (per skill-evolution SKILL.md section 4):
             **Low risk** — auto-apply immediately:
               1. Backup target SKILL.md → `<path>.bak-YYYYMMDD-HHMM`
               2. Apply the diff. If clean apply fails (file modified since trace was captured),
                  downgrade to medium risk and skip auto-apply.
               3. Append to `~/.claude/projects/-home-glc/memory/skill-evolution/changelog.yaml`:
                  ```yaml
                  - date: "YYYY-MM-DD"
                    skill: <skill-name>
                    ...
                    auto_applied: true
                  ```
               4. Move proposal file to `proposals/applied/`.
             **Medium / High risk** — leave proposal file in `proposals/` for user review.
        vi.  Output summary (auto-applied first, then pending):
             [Evolve] auto-applied: <N> (list skill names); pending approval: <M> (list skill names).
        vii. Reset `turn_count_since_last_analysis` to 0, update `last_analysis_date`.
      - If no diagnosis meets the confidence threshold, output:
        [Evolve] N new traces, no proposals (all dimensions below threshold).
        Reset counter to 0.
   d. If `turn_count_since_last_analysis < analysis_interval`, do nothing (no output).
10. Print one-line summary, e.g.:
...
ara/
  ...
  logic/                            # MUTABLE — current best understanding (Stage 4 reconciles)
    ...
    experiments.md
    solution/
      architecture.md               #   Model architecture + component graph
      algorithm.md                  #   Math + pseudocode
      constraints.md                #   Sensor / Environment / Deployment constraints
      heuristics.md                 #   Implementation tricks + rationale + sensitivity
    related_work.md
  src/                              # How (code artifacts)
    configs/
      model.md                      #   Model/architecture hyperparams
      training.md                   #   Training hyperparams with rationale
      augmentation.md               #   Data augmentation configs + rationale
    kernel/
    environment.md                  #   Python, framework, CUDA, hardware, seeds
  trace/                            # APPEND-ONLY — the journey, never rewritten
    ...
    checkpoints.yaml                #   Model checkpoint log: epoch, metrics, path
  ...
  evidence/                         # APPEND-ONLY — raw proof
    README.md
    tables/                         #   Exact result tables from papers/experiments
    figures/                        #   Extracted data points from figures
    benchmarks/                     #   KITTI / Waymo / nuScenes / Argoverse2 standard results
    ablation/                       #   Ablation study results (component, sensor, weather, etc.)
  staging/                          # APPEND-ONLY — unclassified / awaiting closure
    observations.yaml               #   The crystallization buffer
```

### Skill Evolution State (outside ara/)

```
~/.claude/projects/-home-glc/memory/skill-evolution/
  state.yaml                        # Auto-analysis throttle: turn counter + interval
  registry.yaml                     # Skill inventory, versions, instrumentation status
  changelog.yaml                    # Applied skill change history
  traces/
    turns/                          # Layer 1: per-turn skill usage records (research-manager writes)
    <skill-name>/                   # Layer 2: per-skill instrumentation logs
  proposals/                        # Pending edit proposals awaiting user review
```

### Domain-specific directory rationale

- **`evidence/benchmarks/`** — Standardized evaluation protocol results. Each file
  corresponds to one benchmark run on one dataset split (e.g.,
  `waymo_validation_vehicle_level2.md`). Include exact metric values (mAP, mAPH, NDS,
  AMOTA), never rounded.
- **`evidence/ablation/`** — Controlled variable experiments. Files organized by
  ablation dimension: component removal, sensor configuration, weather condition,
  range bin, object size category.
- **`src/configs/augmentation.md`** — Data augmentation is a first-class research
  concern in LiDAR perception (ground-truth sampling, point jitter, beam dropout,
  weather simulation). Document each augmentation with its rationale and sensitivity.
- **`trace/checkpoints.yaml`** — Training trajectory. Each entry records epoch,
  primary metrics, and model path. Enables reconstructing which checkpoint produced
  which claim's evidence.
- **`logic/solution/constraints.md`** — Structured into three categories relevant to
  LiDAR perception (see Schema below).
...
### Model Checkpoint (`trace/checkpoints.yaml`)
...
checkpoints:
  - id: CKPT{XX}
    ...
    metrics:
      # dataset-specific, e.g.:
      vehicle_L2_mAPH: 0.7496
      pedestrian_L2_mAPH: 0.7506
      cyclist_L2_mAPH: 0.7452
    notes: "{why this checkpoint was saved or is notable}"
    bound_to: [N{XX}, ...]    # exploration nodes this checkpoint relates to
```
...
- **Last revised**: YYYY-MM-DD (turn-id)
```
...
Structured into three domain-specific categories:
...
## Sensor Constraints
### SC01: {title}
- **Sensor type**: {LiDAR model, lines, wavelength}
- **Constraint**: {what is limited by the sensor}
- **Quantification**: {numerical bounds, e.g. point density @ range, FoV limits}
- **Impact**: {which claims depend on this constraint}

## Environment Constraints
### EC01: {title}
- **Condition**: {clear / rain / fog / snow / dust / night}
- **Constraint**: {how environment affects sensor or model}
- **Quantification**: {attenuation coefficient, effective range reduction, noise level}
- **Impact**: {which claims are bounded by this condition}

## Deployment Constraints
### DC01: {title}
- **Requirement**: {latency budget, memory limit, power envelope}
- **Constraint**: {what is limited by deployment target}
- **Quantification**: {ms, GB, W, FPS targets}
- **Impact**: {which architectural choices are driven by this}
```
...
```
mkdir -p ara/{logic/solution,src/{configs,kernel},trace/sessions,evidence/{tables,figures,benchmarks,ablation},staging}
```
...
4. `ara/trace/checkpoints.yaml` — `checkpoints: []`
5. `ara/trace/pm_reasoning_log.yaml` — `entries: []`
6. `ara/staging/observations.yaml` — `observations: []`
7. `ara/logic/claims.md` — `# Claims`
8. `ara/logic/problem.md` — `# Problem`
9. `ara/logic/solution/heuristics.md` — `# Heuristics`
10. `ara/logic/solution/constraints.md` — `# Constraints`
11. `ara/evidence/README.md` — `# Evidence Index`
...
- `checkpoints.yaml` last entry (which checkpoint is current)
...
8. **Trace and staging are append-only.** Never edit prior entries in `trace/sessions/`,
   `trace/pm_reasoning_log.yaml`, `trace/exploration_tree.yaml`, `trace/checkpoints.yaml`,
   or `staging/observations.yaml` except to set forward-reference pointers (e.g.
...
16. **Benchmark and ablation results are raw evidence.** Write to `evidence/benchmarks/`
    and `evidence/ablation/` immediately on production — they are not interpretations.
17. **Track checkpoints.** Each model save with notable metrics gets a `trace/checkpoints.yaml`
    entry. Links claims to the model version that produced their supporting evidence.
